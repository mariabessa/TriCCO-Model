...
double cco::getProjectionT(const Vec2& p, const Vec2& a, const Vec2& b){
  ...
  return std::max(0.0, std::min(1.0, t)); // restringe t em [0,1], se a proj ficar fora do segmento usamos o ponto mais próximo (a ou b)
}

// distância do ponto p ao segmento ab
double cco::distPointToSegment(const Vec2& p, const Vec2& a, const Vec2& b){
  double t = getProjectionT(p, a, b);
  Vec2 proj = lerp(a, b, t); // usa lerp para encontrar o ponto projetado
  return dist(p, proj); 
}
